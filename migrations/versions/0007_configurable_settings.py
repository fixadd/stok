...
def upgrade() -> None:
    op.execute(
        ...
        CREATE INDEX ix_setting_lists_scope ON setting_lists(scope);
        CREATE INDEX ix_setting_lists_active ON setting_lists(active);

        CREATE TABLE setting_options (
            id SERIAL PRIMARY KEY,
            setting_list_id INTEGER NOT NULL REFERENCES setting_lists(id) ON DELETE CASCADE,
            label VARCHAR(160) NOT NULL,
        ...
        CREATE INDEX ix_setting_options_list ON setting_options(setting_list_id);
        CREATE INDEX ix_setting_options_active ON setting_options(active);
        ...
        CREATE INDEX ix_field_groups_entity ON field_groups(entity_type);
        CREATE INDEX ix_field_groups_active ON field_groups(active);
        ...
        CREATE TABLE custom_fields (
            ...
            group_id INTEGER REFERENCES field_groups(id) ON DELETE SET NULL,
            required BOOLEAN NOT NULL DEFAULT FALSE,
            active BOOLEAN NOT NULL DEFAULT TRUE,
            visible_form BOOLEAN NOT NULL DEFAULT TRUE,
            visible_list BOOLEAN NOT NULL DEFAULT FALSE,
        ...
        CREATE INDEX ix_custom_fields_entity ON custom_fields(entity_type);
        CREATE INDEX ix_custom_fields_group ON custom_fields(group_id);
        CREATE INDEX ix_custom_fields_active ON custom_fields(active);
        ...
        CREATE INDEX ix_custom_field_options_field ON custom_field_options(field_id);
        ...
        CREATE INDEX ix_custom_field_values_target ON custom_field_values(entity_type, entity_id);
        CREATE INDEX ix_custom_field_values_field ON custom_field_values(field_id);

        CREATE TABLE dashboard_widgets (
            id SERIAL PRIMARY KEY,
            widget_key VARCHAR(128) NOT NULL UNIQUE,
            label VARCHAR(160) NOT NULL,
            widget_type VARCHAR(32) NOT NULL DEFAULT 'metric',
    ...
    seed = {
        "inventory_status": ("Envanter Durumu", "inventory", [
            ("Aktif", "aktif"), ("Beklemede", "beklemede"), ("Arızalı", "arizali"), ("Hurda", "hurda"), ("Stokta", "stokta")]),
    ...
    for key, (label, scope, options) in seed.items():
        op.execute(
            "INSERT INTO setting_lists (key,label,scope) VALUES (%(key)s,%(label)s,%(scope)s) ON CONFLICT (key) DO NOTHING",
            {"key": key, "label": label, "scope": scope},
        )
        for order, (option_label, value) in enumerate(options):
            op.execute(
                """
                INSERT INTO setting_options (setting_list_id,label,value,sort_order)
                SELECT id,%(label)s,%(value)s,%(sort_order)s FROM setting_lists WHERE key=%(key)s
                ON CONFLICT (setting_list_id,value) DO NOTHING
                """,
                {"key": key, "label": option_label, "value": value, "sort_order": order},
            )

...
def test_sanitize_input_text_strips_and_limits():
    ...
    assert sanitize_input_text(123) == "123"


def test_sanitize_metadata_payload_handles_non_dict_and_limits_values():
    assert sanitize_metadata_payload(None) == {}
    assert sanitize_metadata_payload({" key ": " value ", "": "ignored"}) == {"key": "value"}
    assert sanitize_metadata_payload({"x": None}) == {"x": ""}
...
def test_parse_int_or_none_is_strict():
    ...
    assert parse_int_or_none("1e2") is None

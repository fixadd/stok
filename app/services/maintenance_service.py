...
from ..queries import maintenance_queries
from ..services.responses import error, ok
from ..utils.parsing import sanitize_input_text

MAINTENANCE_INTERVAL_DAYS = 90
MAINTENANCE_WARNING_DAYS = 15

COMPUTER_KEYWORDS = {"bilgisayar", "laptop", "desktop", "notebook", "dizustu", "masaustu", "pc"}
...
def _record_payload(record: InventoryMaintenance) -> dict[str, Any]:
    ...
    return {
        "id": record.id, "item_id": record.item_id,
        ...
        "note": record.note or "", "status": status["key"],
        "status_label": status["label"], "status_class": status["class"],
...
def _item_payload(item: InventoryItem) -> dict[str, Any]:
    ...
    return {
        ...
        "maintenance_status": status["label"], "maintenance_status_key": status["key"],
...
def _computer_item(item_id: int) -> InventoryItem | None:
    item = maintenance_queries.get_item(item_id)
...
def create(deps: dict[str, Any], item_id: int, data: Any) -> tuple[dict[str, Any], int]:
    ...
    if item is None: return error("Bakım kaydı yalnızca bilgisayar envanterleri için oluşturulabilir."), 400
    ...
    except ValueError: return error("Bakım tarihi geçerli bir tarih olmalıdır."), 400
    ...
    db.session.commit()
    return ok(maintenance=_record_payload(record)), 201


def list_records(deps: dict[str, Any], item_id: int) -> tuple[dict[str, Any], int]:
    item = _computer_item(item_id)
    if item is None: return error("Bakım geçmişi yalnızca bilgisayar envanterleri için görüntülenebilir."), 400
    records = maintenance_queries.list_records(item.id)
    return ok(item=_item_payload(item), maintenances=[_record_payload(r) for r in records]), 200


def update(deps: dict[str, Any], item_id: int, maintenance_id: int, data: Any) -> tuple[dict[str, Any], int]:
    item = _computer_item(item_id)
    if item is None: return error("Bakım kaydı yalnızca bilgisayar envanterleri için güncellenebilir."), 400
    record = maintenance_queries.get_record(item_id, maintenance_id)
    if record is None: return error("Bakım kaydı bulunamadı."), 404
    if not isinstance(data, dict): return error("Geçersiz JSON gövdesi."), 400
    actor = sanitize_input_text(data.get("performed_by"), max_length=128) or deps["current_actor_name"]()
    note = sanitize_input_text(data.get("note"), max_length=2000) or None
    try: performed_at = datetime.fromisoformat(str(data.get("performed_at") or "").strip())
    except ValueError: return error("Bakım tarihi geçerli bir tarih olmalıdır."), 400
    record.performed_at = performed_at; record.performed_by = actor; record.note = note
    next_at = _next_maintenance(performed_at)
    ...
    db.session.commit()
    return ok(maintenance=_record_payload(record)), 200


def delete(deps: dict[str, Any], item_id: int, maintenance_id: int) -> tuple[dict[str, Any], int]:
    item = _computer_item(item_id)
    if item is None: return error("Bakım kaydı yalnızca bilgisayar envanterlerinden silinebilir."), 400
    record = maintenance_queries.get_record(item_id, maintenance_id)
    if record is None: return error("Bakım kaydı bulunamadı."), 404
    ...
    return ok(message="Bakım kaydı silindi.", maintenance_id=maintenance_id), 200


def load_payload(deps: dict[str, Any]) -> dict[str, Any]:
    computers = []
    for item in maintenance_queries.list_items():
        if not is_computer_hardware_type(item.hardware_type.name if item.hardware_type else None): continue
        if (item.status or "").lower() in {"hurda", "stokta"}: continue
        computers.append(_item_payload(item))
    return {
        "maintenance_items": computers, "maintenance_total_count": len(computers),

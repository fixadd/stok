...
def load_maintenance_metrics() -> dict[str, int]:
    items = InventoryItem.query.filter(func.lower(InventoryItem.status).notin_(["hurda", "stokta"])).all()
    ...
    for item in items:
        ...
        if not records:
            none_count += 1
            continue
        status = calculate_maintenance_status(records[0].performed_at)["status"]
        if status == "overdue":
            overdue_count += 1; due_count += 1
        elif status == "warning":
            warning_count += 1; due_count += 1
        else:
            current_count += 1
    return {
...
def load_dashboard_metrics() -> dict[str, Any]:
    available_stock = StockItem.query.with_entities(func.sum(StockItem.quantity)).filter(StockItem.status == "stokta").scalar() or 0
    ...
    critical_stock_count = StockItem.query.filter(StockItem.status.in_(["arizali", "hurda"])).count()
    maintenance_counts = load_maintenance_metrics()
    recent_stock_movements = StockMovement.query.options(joinedload(StockMovement.stock_item)).order_by(StockMovement.created_at.desc()).limit(5).all()

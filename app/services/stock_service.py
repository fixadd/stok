...
def move_inventory_to_stock(deps: dict[str, Any], item_id: int, data: dict[str, Any]):
    db = deps["db"]
    ...
    if existing_stock:
        ...
        existing_stock.metadata_payload = {key: value for key, value in metadata_payload.items() if value}
...
def scrap_inventory(deps: dict[str, Any], item_id: int, data: dict[str, Any]):
    db = deps["db"]
    get_inventory_item_with_relations = deps["get_inventory_item_with_relations"]
    serialize_inventory_item = deps["serialize_inventory_item"]
    add_inventory_event = deps["add_inventory_event"]
...
def move_license_to_stock(deps: dict[str, Any], license_id: int, data: dict[str, Any]):
    ...
    note = sanitize_input_text(data.get("note"), max_length=512)
    actor = sanitize_input_text(data.get("performed_by")) or deps["DEFAULT_EVENT_ACTOR"]
    associated_item = license_record.item
    stock_item = create_stock_item_from_license(license_record, note=note, actor=actor)
    ...
    fresh_license = (
        ...
        .filter_by(id=license_record.id)
        .first()
    )
...
def create_stock_entry(deps: dict[str, Any], data: dict[str, Any]):
    db = deps["db"]
    StockItem = deps["StockItem"]
    ...
    if not reference_code:
        reference_code = metadata_payload.get("inventory_no") or metadata_payload.get("license_key") or None

...
from ..models import CustomField, CustomFieldOption, FieldGroup, SettingList, SettingOption, db
...
def register_settings_routes(app, deps=None):
    ...
    def page():
        ...
        return render_template("settings.html", active_page="settings", setting_lists=lists, custom_fields=fields, field_groups=groups, field_types=FIELD_TYPES)
    ...
    def api_add_option(list_id: int):
        ...
        try:
            ...
            record_activity(area="sistem", action="ayar_secenegi_ekle", description=f"{option.label} seçeneği güncellendi.", actor=actor.username if actor else "Sistem")
            db.session.commit()
    ...
    def api_toggle_option(option_id: int):
        ...
        if option is None: return jsonify({"error": "Seçenek bulunamadı."}), 404
        option.active = bool(data.get("active", True)); db.session.commit()
    ...
    def api_add_field():
        if (error := require_admin()): return error
        data = request.get_json(silent=True) or request.form
        entity_type, field_key, label, field_type = (str(data.get(k, "")).strip() for k in ("entity_type", "field_key", "label", "field_type"))
        entity_type = entity_type or "inventory"; field_type = field_type or "text"
        ...
        field = CustomField(entity_type=entity_type, field_key=field_key, label=label, field_type=field_type,
            required=bool(data.get("required", False)), active=True, visible_form=bool(data.get("visible_form", True)),
            visible_list=bool(data.get("visible_list", False)), searchable=bool(data.get("searchable", False)),
            sortable=bool(data.get("sortable", False)), placeholder=str(data.get("placeholder", "")).strip() or None,
            help_text=str(data.get("help_text", "")).strip() or None, default_value=str(data.get("default_value", "")).strip() or None,
            sort_order=int(data.get("sort_order", 0) or 0))
        db.session.add(field)
        actor = get_active_user(); record_activity(area="sistem", action="ozel_alan_ekle", description=f"{label} alanı oluşturuldu.", actor=actor.username if actor else "Sistem")
        db.session.commit()
    ...
    def api_update_field(field_id: int):
        ...
        for key in ("label", "placeholder", "help_text", "default_value", "field_type"):
            if key in data:
                if key == "field_type" and data[key] not in FIELD_TYPES: return jsonify({"error": "Geçersiz alan tipi."}), 400
                setattr(field, key, str(data[key]).strip())
        for key in ("required", "active", "visible_form", "visible_list", "searchable", "sortable"):
            if key in data: setattr(field, key, bool(data[key]))
        if "sort_order" in data: field.sort_order = int(data["sort_order"])
        db.session.commit(); return jsonify({"ok": True, "field": serialize_custom_field(field)})
    ...
    def api_add_field_option(field_id: int):
        ...
        field = db.session.get(CustomField, field_id)
        if field is None: return jsonify({"error": "Alan bulunamadı."}), 404
        data = request.get_json(silent=True) or {}; label = str(data.get("label", "")).strip(); value = str(data.get("value", label)).strip().lower().replace(" ", "_")
        ...
        option = CustomFieldOption(field_id=field_id, label=label, value=value, sort_order=int(data.get("sort_order", 0) or 0)); db.session.add(option); db.session.commit()
        return jsonify({"ok": True, "id": option.id, "label": option.label, "value": option.value})

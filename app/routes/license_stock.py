...
def register_license_stock_routes(app, deps=None):
    ...
    def move_license_to_stock_api(license_id: int):
        ...
        actor = sanitize_input_text(data.get("performed_by")) or current_actor_name() or "Sistem"

        # Ortak stok helper'larını istek anında içeri alıyoruz. Böylece route
        # modülü uygulama başlatılırken circular import oluşturmuyor.
        from .. import (
            add_inventory_event,
            create_stock_item_from_license,
            get_stock_item_with_relations,
            serialize_license_record,

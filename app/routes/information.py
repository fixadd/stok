...
def register_information_routes(app, deps):
    ...
    def create_information_entry():
        ...
        image_filename = deps["save_information_image"](
    ...
    def information_edit(entry_id: int):
        ...
        if request.method == "POST":
            title = (request.form.get("title") or "").strip()
            category_id = deps["parse_int_or_none"](
            ...
            if not title or not category_id or not content:
                flash(
                    "Lütfen başlık, kategori ve içerik alanlarını doldurun.",
                    "danger",
                )
                return redirect(
                    url_for("information_edit", entry_id=entry.id)
                )
            ...
            new_filename = deps["save_information_image"](
                request.files.get("photo")
            )

            if new_filename:
                deps["remove_information_image"](

...
NAV_SECTIONS = OrderedDict([("general", "Genel"), ("inventory", "Envanter"), ("operations", "İşlemler"), ("settings", "Ayarlar")])
...
def build_sidebar_sections(user, active_page: str | None, endpoint: str | None) -> list[dict[str, Any]]:
    ...
    for section_key, section_title in NAV_SECTIONS.items():
        ...
        for key, item in NAV_ITEMS.items():
            if item["section"] != section_key or not has_system_role(user, item["min_role"]):
                continue
            items.append({"key": key, "title": item["title"], "endpoint": item["endpoint"], "icon": item["icon"], "is_active": key == active_key})
        if items:
